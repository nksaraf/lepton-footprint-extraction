...
def _iter_valid_files(directory, white_list_formats, follow_links):
    ...
    for root, subdirs, files in _recursive_list(directory):
        output_dir_idx = subdirs.find(OUTPUT_DIR)
        if output_dir_idx == -1:
            warnings.warn('No output(masks) folder found in ' + root)
        ...
        for file in files:
            ...
            if ext in white_list_formats:
                if output_dir_found and os.path.exists(os.path.join(os.path.join(root, OUTPUT_DIR), '{}{}.{}'.format(fname, OUTPUT_SUFFIX,ext)))
