...
class LeptonDataGenerator(object):
    def __init__(self,
                rotation_angles=0,
                width_shift_range=0.,
    ...
    def flow_from_directory(self, directory,
        ...
        return DirectoryIterator(
            directory, self,
            target_size=target_size, color_mode=color_mode, class_mode=class_mode,
    ...
    def get_random_transform(self, img_shape, seed=None):
        ...
        if self.rotation_angles:
            theta = np.random.choice(self.rotation_angles) #degrees
        else:
            theta = 0 
        ...
        for file in files:
            ...
            if ext in white_list_formats:
                ...
                if os.path.exists(os.path.join(root, output_filename)) 
                    or os.path.exists(os.path.join(os.path.join(root, OUTPUT_DIR), output_filename)):
                    yield root, file
...
def _count_valid_files_in_directory(directory,
    ...
    # Returns
        ...
        the directory.
    """
    num_files = len(list(_iter_valid_files(directory, white_list_formats)))
    if split:
...
class DirectoryCollectionIterator(Iterator):
    """Iterator capable of reading images from a directory on disk.

    # Arguments
        directories: list of directories to read images from.
        image_data_generator: Instance of `ImageDataGenerator`
            to use for random transformations and normalization.
    ...
    def __init__(self, directories, image_data_generator,
                 target_size=(256, 256), color_mode='rgb',
                 class_mode='categorical',
                 batch_size=32, shuffle=True, seed=None,
                 save_to_dir=None, save_prefix='', save_format='png',
        ...
        for directory in sorted(directories):
            if os.path.isdir(directory):
                ...
                warnings.warn('No directory found with path {}'.format(directory))
        ...
        for i, res in enumerate(results):
            filenames = res.get()
            print('Found {} images in {}'.format(len(filenames), self.directories[i]))
        ...
        if self.save_to_dir:
            for i, j in enumerate(index_array):
                fname = '{prefix}_{index}_{hash}.{format}'.format(
                    ...
                    format=self.save_format)
                save_img(os.path.join(self.save_to_dir, fname), img)
